...
export const Container = styled(TouchableOpacity).attrs<Props>({
    ...
    background-color: ${({ theme, type }) => type === "PRIMARY" ? theme.COLORS.GREEN_LIGHT : type === "SECONDARY" ? theme.COLORS.RED_LIGHT : theme.COLORS.GRAY_6};
    border: 1px solid ${({ theme, type }) => type === "PRIMARY" ? theme.COLORS.GREEN_DARK : type === "SECONDARY" ? theme.COLORS.RED_DARK : theme.COLORS.GRAY_6};
    border-radius: 6px;
    flex-direction: row;
    align-items: center;
